...
The site is now live (or ready to go live — see "Deploying" below), and every page carries a small notice banner naming what's still unfinished — **remove that banner once it's resolved**. Address/phone/email/hours, Footscan/Orthotics/Biomechanical pricing, the Carl Bell/Kate Disley bios and photos, and all section imagery are already real (see above). The fictional testimonials that used to sit on `index.html` were removed rather than published as real customer quotes — add a "Client stories" section back once there are genuine reviews to show (the `.testimonial-card` styles are still in `css/style.css`, ready to reuse).

- **Social links** in the footer — Facebook is real (`https://www.facebook.com/share/1DdbLqZMXb/?mibextid=wwXIfr`, opens in a new tab via `target="_blank" rel="noopener"`) across all 10 pages. Instagram is still a placeholder `#` link — add the real URL when there is one, or remove that icon.
- **Contact form** — `contact.html`'s general enquiry form now really submits, via [FormSubmit.co](https://formsubmit.co) (`action="https://formsubmit.co/ajax/info@momentumsic.com"`, called with `fetch` from `js/script.js` so the existing inline success/error UI still works, no page redirect). FormSubmit needs **no signup**, but the very first submission to a new address only triggers a confirmation email to `info@momentumsic.com` — it does **not** get delivered as a real enquiry. **Before removing the banner:** submit a test message through the live form yourself, find FormSubmit's "please confirm" email in that inbox (check spam) and click the confirmation link, then submit a second test message and confirm it actually arrives. Until that's done, real enquiries silently won't reach anyone — the form's error box (and the banner) are the safety net telling visitors to call/email directly instead. `book-appointment.html` still has no form of its own — booking goes through Momentum's system instead (see above).
